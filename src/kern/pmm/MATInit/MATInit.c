...
pmem_init(unsigned int mbi_addr)
{
  ...
  // TODO
  for(it = 0; it < size; it++){
    if(get_mms(it) + get_mml(it) > highest_address){
      highest_address = get_mms(it) + get_mml(it);
    }
  }

  // if(highest_address % PAGESIZE == 0){
  //   nps = highest_address / PAGESIZE;
  // }
  // else nps = highest_address / PAGESIZE + 1;
  ...
  /** TASK 2:
    ...
    */
  // TODO
    for(it = 0; it < VM_USERLO_PI; it++) at_set_perm(it, 1);
    for(it = VM_USERHI_PI; it < nps; it++) at_set_perm(it, 1);
    for(it = VM_USERLO_PI; it < VM_USERHI_PI; it++){
      // if(is_usable(it)) at_set_perm(it, 2);
      // else at_set_perm(it, 0);
      at_set_perm(it,2);
    }
