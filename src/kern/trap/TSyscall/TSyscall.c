...
void sys_spawn(void)
{
  ...
  if(exec == 1)
  {
    unsigned int pid = proc_create(_binary___obj_user_pingpong_ping_start , quota);
    syscall_set_errno(E_SUCC);
    syscall_set_retval1(pid);
  }
  else if(exec == 2)
  {
    unsigned int pid = proc_create(_binary___obj_user_pingpong_pong_start , quota);
    syscall_set_errno(E_SUCC);
    syscall_set_retval1(pid);
  }
  else if(exec == 3)
  {
    unsigned int pid = proc_create(_binary___obj_user_pingpong_ding_start , quota);
    syscall_set_errno(E_SUCC);
    syscall_set_retval1(pid);
  }
  else if(exec == 4)
  {
    unsigned int pid = proc_create(_binary___obj_user_fork_fork_start , quota);
    syscall_set_errno(E_SUCC);
    syscall_set_retval1(pid);
  }
  else
